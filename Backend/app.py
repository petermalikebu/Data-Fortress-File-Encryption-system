...
ALLOWED_EXTENSIONS = {'txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif', 'docx', 'csv', 'pptx', 'mp4', 'mp3', 'mpeg', 'mkv'}
...
def send_email_notification(recipient_email, filename, sender_email):
    ...
    message = Mail(
        ...
        <p>Thank you,<br>DataFortress Site</p>
        """
    )
...
def view_audit_logs():
    if 'email' in session:
        ...
        if request.method == 'POST':
            # Handle CSV download
            return export_audit_logs_csv()  # Directly call the export function
